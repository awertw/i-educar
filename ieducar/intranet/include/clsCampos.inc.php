...
class clsCampos extends Core_Controller_Page_Abstract
{
  ...
  function tabela(name, counter)
  {
    ...
    this.removeRow = function(row)
    {
      ...
      for (var ct = 2; ct < divs.length - 1; ct++) {
        if (divs[ct] == tr) {
          tab.deleteRow(ct);
        }
      }

      divs = document.getElementsByName(\'tr_\'+This.nome + \'[]\');
      var classe = \'formmdtd tr_\' + This.nome;

      for (var ct = 0; ct < divs.length; ct++) {
        if (divs[ct] && divs[ct].id != \'adicionar_linha\') {
          divs[ct].className = classe;

          for (var c = 0; c < divs[ct].cells.length; c++)
            divs[ct].cells[c].className = classe;
    ...
    this.setId = function setId(tab)
    {
      var divs = tab.getElementsByTagName(\'TR\');
      var cod_ini = 0;

      for (var ct = 2; ct < divs.length; ct++) {
        var nome_tr = /[a-zA-Z-_]*/.exec(divs[ct].id);

        if (!nome_tr)
           continue;

         divs[ct].setAttribute( "id",nome_tr + \'[\' + cod_ini + \']\');

        for (var c = 0; c < divs[ct].cells.length; c++) {
          var nome_td = /[a-zA-Z-_]*/.exec(divs[ct].cells[c].id);
          divs[ct].cells[c].setAttribute( "id",nome_td + \'[\' + cod_ini + \']\');

          var campos = divs[ct].cells[c].childNodes;
  ...
  ';
        ...
        foreach ($arr_campos as $nome => $componente) {
            ...
            if (preg_match('/^(tab_add_[0-9]+)/', $nome) === 1) {
                ...
                $retorno .= "<div id='tr_$nome_tabela' class='$classe'><div style='width: 100%'></div></div>";
                $retorno .= "\n<div  id='$nome_tabela' class='tabela-adicao container' ><div class='row'>";
                ...
                if ($titulo) {
                    $retorno .= "<div id='tr_{$nome_tabela}_tit' style='font-weight:bold'  class='formdktd'><div>$titulo</div></div>";
                } else {
                    $retorno .= "<div id='tr_{$nome_tabela}_tit' style='font-weight:bold;display:none;visibility:hidden;' ><div >&nbsp;</div></div>";
                }

                $retorno .= "<div style='font-weight:bold' id='tr_{$nome_tabela}_cab'>";
                ...
                foreach ($cabecalho as $key => $cab) {
                    ...
                    $retorno .= "<div class='$classe2' id='td_$cabId'><span class='form colorFont'>$cab</span>{$obrigatorio}";
                    $retorno .= "<div class='$class2' placeholder='teste' id='tr_{$nome_tabela}[$key2]'></div";
                    $retorno .= "<div>".$this->getCampoLista("{$nome}[{$key2}]", "{$nome}[$key2]", $campo_[5], $lista, $valor[$key], $campo_[7], $campo_[8], $class, $campo_[9])."</div>";
                    $retorno .= "</div></div>";
                }

                $retorno .= '<div class=\'formmdtd\' id=\'td_acao\' ><span class=\'form\'>A&ccedil;&atilde;o</span></div>';
                $retorno .= '</div>';
                ...
                foreach ($valores as $key2 => $valor) {
                    ...
                    $retorno .= "<div id='tr_{$nome_tabela}[$key2]' name='tr_{$nome_tabela}[]'  $style class='$classe2 tr_{$nome_tabela}'>";
                    $array_valores_lista = $valores_lista_tabela[$key2];
                    ...
                    foreach ($componente as $key => $campo_) {
                        ...
                        $retorno .= "<div class='$classe2 {$nome}' id='td_{$nome}[{$key2}]'>\n";
                    ...
                    $retorno .= "<div><a href='javascript:void(0)' onclick='$click' id='link_remove[$key2]' style='outline: none;'>$img</a>$retorno_oculto</div>";
                    $img = '<img src="/intranet/imagens/banco_imagens/excluirrr.png" border="0" alt="excluir" />';
                    $id = count($valores);
                ...
                $click = "$nome_add.addRow();";
                $img = '<img src="/intranet/imagens/nvp_bot_novo.png" border="0" alt="incluir" style="float:left; margin:5px;" />';
                $retorno .= '<div id=\'adicionar_linha\' style="background-color:#f5f9fd;">';
                $tt = $total_campos + 1;
                $retorno .= "<div style='padding-top: 17px !important;'><label><a style=\"text-decoration:none;\" href='javascript:void(0)' id='btn_add_$nome_add' onclick='$click' style='outline: none;'><p>$img ADICIONAR NOVO</p></a></label></div>";
                $retorno .= '</div>';
                $retorno .= '</div>';
