function ajax(funcaoRetorno) {
    ...
    if (window.XMLHttpRequest) {
        try {
            ...
            xml.envia = function() {
                xml.open("GET", addRandToURL(arguments[0]), true);
                xml.send(null)
            };
            return xml;
        } catch (e) {
    ...
    // pro IE :/
        ...
        try {
            ...
            This.envia = function() {
                This.xml.open("GET", addRandToURL(arguments[0]), true);
                This.xml.send()
            };
        } catch (e) {
            alert("Erro ajax: " + e.description);
...
function centralizaExpansivel() {
    ...
    for (let i = 0; i < DOM_divs.length; i++) {
        ...
        expansivel.style.top = 'calc(50% - ' + (altura / 2) + 'rem)';
        expansivel.style.left = 'calc(50% - ' + (largura / 2) + 'rem)';
    }
}
...
function showExpansivel(largura, altura, conteudo) {
    ...
    */
    if (altura != 0) {
        expansivel.style.height = altura + 'rem';
        document.getElementById("expansivel_conteudo").style.height = altura + 'rem';
    }
    ...
    if (largura != 0) {
        expansivel.style.width = largura + 'rem';
        document.getElementById("expansivel_conteudo").style.width = largura + 'rem';
    } else {
