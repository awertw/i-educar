...
const btnDesativar = `
    <button
        id='cod_turma_checkbox[#aaytu#]'
        name='cod_turma_checkbox[]'
        style='width: 80px;'
        class='btn btn-info'
...
function handleCadastrarUsuarioServidor (response) {
    if (!isNaN(response.result.cod_servidor) && !isNaN(response.result.matricula)) {
        span_btn_parent = document.getElementById(`cod_turma_checkbox[${response.result.cod_servidor}]`).parentElement;
...
function handleDesativarUsuarioServidor (response) {
    if (!isNaN(response.result)) {
        span_btn_parent = document.getElementById(`cod_turma_checkbox[${response.result}]`).parentElement;
        span_btn_parent.innerHTML = '';
