  ...
  $(document).ready(function () {
    ...
    function getComponentesCurriculares() {
      if (ano && turma_id) {
        ...
        var urlForGetComponentesCurriculares = getResourceUrlBuilder.buildUrl(
          "/module/DynamicInput/componenteCurricular",
    ...
    async function getObjetivosAprendizagem() {
      $(titleTable)
        .children()
        .first()
        .html("Aguarde, carregando Objetivo(s) de aprendizagem...");

      await getComponentesCurriculares();

      var urlForGetObjetivosAprendizagem = getResourceUrlBuilder.buildUrl(
        "/module/Api/PlanejamentoAula",
        "get-objetivos-aprendizagem",
        {}
      );
    ...
    function fillComponenteCurricular(index, response) {
      let ccElement = document.getElementById(
        `ref_cod_componente_curricular_array[${index}]`
      );
      $.each(componentesCurriculares, function (id, mapValue) {
        ...
        if (id.indexOf && id.substr && id.indexOf("__") == 0) {
          ...
          $(ccElement).append(
            `<option value="${id}" ${selected}>${mapValue.value}</option>`
          );
        }
    ...
    function fillHabilidadesAndEspecificacoes(index, response) {
      ...
      let bnccEspecificoesElemento = document.getElementById(
        `custom_bncc_especificacoes[${index}]`
      );
      let habilidadesGeralCC = Object.entries(
        response[index].habilidades.habilidades_geral_cc
      );
      ...
      habilidadesGeralCC.forEach(function (habilidade, key) {
        ...
        if (
          $.inArray(
            id,
            response[index].habilidades.habilidades_planejamento_aula_cc
          ) !== -1
        ) {
          selected = "selected";
        }
        ...
        if (response[index].especificacoes.especificacoes_geral_bncc[id]) {
          let escpecificacoesGeralBNCC = Object.entries(
            response[index].especificacoes.especificacoes_geral_bncc[id]
          );

          escpecificacoesGeralBNCC.forEach(function (especificacao) {
            let selectedEspecificacao = "";
            let idEspecificacao = especificacao[0];
            let valueEspecificacao = especificacao[1]
              .substring(0, maxCharacters)
              .trimEnd();
            valueEspecificacao =
              valueEspecificacao.length < maxCharacters
                ? valueEspecificacao
                : valueEspecificacao.concat("...");

            if (
              $.inArray(
                parseInt(idEspecificacao),
                response[index].especificacoes.especificacoes_pa_bncc[0]
              ) !== -1
            ) {
              selectedEspecificacao = "selected";
            }

            $(bnccEspecificoesElemento).append(
              `<option value="${idEspecificacao}" ${selectedEspecificacao}>${valueEspecificacao}</option>`
            );
    ...
    function consertarBNCCEspecificoesElementos() {
      ...
      index = 0;
      while (index !== count) {
        var bncc_especificaoes = document.getElementById(
          `custom_bncc_especificacoes[${id}]`
        );
    ...
    function handleGetComponentesCurriculares(response, clearComponent = true) {
      ...
      var linhasElemento = document.getElementsByName(
        "tr_objetivos_aprendizagem[]"
      );
      var componentesCurricularesElementos = [];

      // get disciplines elements
      linhasElemento.forEach((linhaElemento) => {
        componentesCurricularesElementos.push(
          linhaElemento.children[0].children[0]
        );
      });

      componentesCurricularesElementos.forEach(
        (componenteCurricularElemento) => {
          var jComponenteCurricularElemento = $(componenteCurricularElemento);

          if (clearComponent) {
            $(componenteCurricularElemento).empty();
            var optionOne =
              '<option id="ref_cod_componente_curricular_array[0]_" value="">Selecione o componente curricular</option>';
          ...
          // bind onchange event
          componenteCurricularElemento.addEventListener(
            "change",
            trocaComponenteCurricular,
            false
          );
        }
      );
    ...
    async function trocaComponenteCurricular(event) {
      ...
      var bnccElemento = document.getElementById(
        `custom_bncc[${componenteCurricularId}]`
      );

      if (turma !== null && componenteCurricularValue !== null) {
        var searchPathBNCCTurma =
            "/module/Api/BNCC?oper=get&resource=bncc_turma",
          paramsBNCCTurma = {
        ...
        await $j.get(
          ...
          function (dataResponse) {
            bnccDados =
              dataResponse.bncc === null
                ? []
                : Object.entries(dataResponse.bncc);
    ...
    async function trocaBNCC(bnccElementoId, bnccArray) {
      ...
      var bnccEspecificoesElemento = document.getElementById(
        `custom_bncc_especificacoes[${bnccElementoId}]`
      );
      ...
      if (
        ...
        var searchPathBNCCEspeficacoesTurma =
            "/module/Api/BNCCEspecificacao?oper=get&resource=list",
          paramsBNCCEspecificacoesTurma = {
            bnccArray: bnccArray,
          };

        await $j.get(
          searchPathBNCCEspeficacoesTurma,
          paramsBNCCEspecificacoesTurma,
          function (dataResponse) {
            var obj = dataResponse.result;
            bnccEspeficacoesDados =
    ...
    function tentaEditarPlanoAula() {
      ...
      var urlForVerificarPlanoAulaSendoUsado = postResourceUrlBuilder.buildUrl(
        "/module/Api/PlanejamentoAula",
        "verificar-plano-aula-sendo-usado-conteudo",
        {}
      );
    ...
    function ehComponentesCurricularesValidos(componentesCurriculares) {
      return componentesCurriculares.every(
        (componenteCurricular) => !isNaN(parseInt(componenteCurricular[1], 10))
      );
    }

    function componentesCurricularesPreenchidos(
      componentesCurriculares,
      componentesCurricularesGeral
    ) {
      ...
      return (
        JSON.stringify(componentesCurricularesFiltrados) ==
        JSON.stringify(componentesUnique)
      );
    }
    ...
    function ehBNCCEspecificacoesValidos(bnccEspecificacoes) {
      return bnccEspecificacoes.every(
        (bnccsEspecificacao) => bnccsEspecificacao[1].length > 0
      );
    }

    function ehConteudosValidos(conteudos) {
      return conteudos.every(
        (conteudo) => conteudo[1] !== "" && conteudo[1] != null
      );
    }
    ...
    function pegarComponentesCurriculares() {
      ...
      tr_objetivos_aprendizagens = document.getElementsByName(
        "tr_objetivos_aprendizagem[]"
      );
    ...
    function pegarBNCCEspecificacoes() {
      ...
      tr_objetivos_aprendizagens.forEach((tr_objetivos_aprendizagem) => {
        ...
        var BNCCEspecificacaoValores = Array.from(
          BNCCEspecificacaoElemento.selectedOptions
        ).map(({ value }) => value);
    ...
    function pegarComponentesCurricularesGeral() {
      ...
      let linhasElemento = document.getElementsByName(
        "tr_objetivos_aprendizagem[]"
      );
    ...
    function editarPlanoAula() {
      let data_inicial = dataParaBanco(
        document.getElementById("data_inicial").value
      );
      let data_final = dataParaBanco(
      ...
      let obrigatorio_conteudo = document.getElementById(
        "obrigatorio_conteudo"
      ).value;
      ...
      console.log(
        obrigatorio_conteudo.length == 1 && obrigatorio_conteudo == "1"
      );
      ...
      if (atividades == null) {
        alert("O campo atividades não é válido.");
        return;
      }
      if (referencias == null) {
        alert("O campo referências não é válido.");
        return;
      }
      ...
      if (
        !componentesCurricularesPreenchidos(
          componentesCurriculares,
          componentesCurricularesGeral
        )
      ) {
      ...
      if (
        obrigatorio_conteudo.length == 1 &&
        obrigatorio_conteudo == "1" &&
        !ehConteudosValidos(conteudos)
      ) {
      ...
      if (registro_adaptacao == null) {
        alert("O campo registro de adaptação não é válido.");
        return;
      }

      var urlForEditarPlanoAula = postResourceUrlBuilder.buildUrl(
        "/module/Api/PlanejamentoAula",
        "editar-plano-aula",
        {}
      );
        ...
        delay(1000).then(() =>
          urlHelper(
            "http://" +
              ...
              "/intranet/educar_professores_planejamento_de_aula_lst.php",
            "_self"
          )
        );
      } else {
    ...
    function openModal() {
      ...
      $j("#dialog-warning-editar-plano-aula")
        .find("#msg")
        .html(getMessageEditarPlanoAula(quantidadeRegistrosAula));
    ...
    function verRegistrosAula() {
      for (let index = 0; index < registrosAula.length; index++) {
        ...
        const url =
          "http://" +
          window.location.host +
          "/intranet/educar_professores_frequencia_cad.php?id=" +
          registroAula;
    ...
    $j("body").append(
      '<div id="dialog-warning-editar-plano-aula' +
        '" style="max-height: 80vh; width: 820px; overflow: auto;">' +
