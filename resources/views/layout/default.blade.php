...
<head>
    ...
    <link rel="stylesheet" type="text/css"
        href="{{ Asset::get('/intranet/scripts/summernote/summernote-lite.css') }}">
...
<body>
    @if (!empty($config['app']['gtm']['id']))
        ...
        <noscript>
            <iframe src="https://www.googletagmanager.com/ns.html?id={{ $config['app']['gtm']['id'] }}" height="0"
    ...
    <script type="text/javascript" src="{{ Asset::get('/intranet/scripts/jquery-maxlength/jquery.plugin.min.js') }}">
    </script>
    <script type="text/javascript" src="{{ Asset::get('/intranet/scripts/jquery-maxlength/jquery.maxlength.min.js') }}">
    </script>
    <script>
        getNotifications();
        ...
        if (window.useEcho) {
            startListenChannel(
                'ieducar-{{ \DB::getDefaultConnection() }}-notification-{{ md5($loggedUser->personId) }}');
        }
    </script>
